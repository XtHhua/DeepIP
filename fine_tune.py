...
def fit(
    model: nn.Module, index: int, optimizer: torch.optim.Optimizer, device: torch.device
):
    model.to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default=f"./log/fine_tune/cifar100/")
    ...
    for i in range(10):
        base_index = choices.index(opt_type) * 10
        base_index += i
        ...
        if opt_type == "all":
            ...
            optimizer = SGD(
                tea_model.classifier.parameters(),
                lr=args.lr,
                momentum=args.momentum,
                weight_decay=args.weight_decay,
                nesterov=args.nesterov,
        ...
        fit(model=tea_model, index=base_index, optimizer=optimizer, device=device)
