...
def fit(model: torch.nn.Module, args: Namespace, device: torch.device):
    model.to(device)
    ...
    for epoch_id in tqdm(range(args.epochs)):
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            model_dir = log_dir.replace("log", "model")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log_dir", type=str, default="./log/source/cifar100")
    ...
    model.classifier[6] = nn.Linear(num_features, args.classes)

    fit(model=model, args=args, device=device)
