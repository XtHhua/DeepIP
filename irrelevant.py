...
def fit(
    ...
    save_dir = f"{PROJECT_ROOT_DIR}/model/irrelevant/{args.dataset}/"
    ...
    for i in range(20):
        if i < 5:
            ...
            model.classifier[-1] = torch.nn.Linear(
                in_feature, len(test_dataset.classes)
            )
        elif 5 <= i < 10:
            model = models.resnet18(weights=True)
            in_feature = model.fc.in_features
            model.fc = torch.nn.Linear(in_feature, len(test_dataset.classes))
        elif 10 <= i < 15:
            model = models.densenet121(weights=True)
            in_feature = model.classifier.in_features
            model.classifier = torch.nn.Linear(in_feature, len(test_dataset.classes))
        elif 15 <= i < 20:
