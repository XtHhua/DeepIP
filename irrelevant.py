...
def fit(model: torch.nn.Module, args: Namespace, index: int, device: torch.device):
    model.to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_dir", type=str, default=f"./log/model_extract_l/cifar100/"
    )
    ...
    for i in range(5):
        ...
        if base_index < 5:
            ...
            in_feature = model.classifier[-1].in_features
            model.classifier[-1] = nn.Linear(in_feature, args.classes)
        fit(model=model, args=args, index=base_index, device=device)
