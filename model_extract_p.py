...
def train(
    ...
    for batch_idx, batch_data in enumerate(data_loader):
        ...
        loss = torch.nn.KLDivLoss()(
            F.log_softmax(output / T, dim=1), F.softmax(t_output / T, dim=1)
        ) * (alpha * T * T) + (1 - alpha) * F.cross_entropy(output, pred)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_dir", type=str, default=f"./log/model_extract_p/cifar100/"
    )
